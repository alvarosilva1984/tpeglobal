...
export class NewscheduleComponent {
  ...
  ngOnInit() {
    ...
    this.authService.getValidity().subscribe((validities) => {
      ...
      let validitiessort = this.validities;
  ...
  carregaLayout() {
    let datamoment = moment.utc(this.diaatual).format("DD-MM-YYYY");
    let diasemana = moment
      .utc(this.diaatual)
      .locale("pt-br")
      .format("dddd");
    let ano = this.diaatual.year();
    let mes = this.diaatual.month();
    let diadia = this.diaatual.date();
    let escala = {
      dia: datamoment,
      diasemana: diasemana,
    ...
    especial = this.especiais.filter(a => {
      let x = new Date(a.begin);
      let y = new Date(a.end);
      let z = escala.data;
      console.log(x);
      console.log(y);
      console.log(z);
    ...
    /*   let inicio = moment.utc(a.begin).format("YYYY-MM-DD");
          let fim = moment.utc(a.end).format("YYYY-MM-DD");
    
          console.log(
            `especial: atual ${this.diaatual.utc().format()} inicido ${moment
              .utc(inicio)
              .format()} fim ${moment.utc(fim).format()}`
          );
          if (
            this.diaatual.utc().isSameOrAfter >= moment.utc(inicio) &&
            this.diaatual.utc() <= moment.utc(fim)
          )
            return true;
          else false;  */
    this.horas.map(h => {
      ...
      this.pontos_dia.map(a => {
        for (let m = 0; m < a.config[this.dayoption].length; m++) {
          if (a.config[this.dayoption][m] == h.code) {
            let pubs = [];
            let config = a.config;
            pontos_hora.push(new Ponto(a.name, a.npubs, a.date, a.id, pubs, config, a.address, a.obs, a.fileimg));
          }
        }
      });
      agendahour = [];
      ...
      for (let i = 0; i < pontos_hora.length - 1; i++) {
        let j = i + Math.floor(Math.random() * (pontos_hora.length - i));
        let temp = pontos_hora[j];
            ...
            if (user.length > 0) {
              ...
              for (let g = 0; g < this.usersesc.length; g++) {
                ...
                if (user[0].userId == this.usersesc[g].userId) {
                  ...
                  let newuser = this.makeNewUser(this.usersesc[g]);
                  newuser.tipoesc = "A";
                  p.pubs[s] = newuser;
                  let userh = this.usersesc.splice(g, 1);
                  ...
                  let dia1semana = moment.utc(this.dia).startOf("isoWeek");
                  let dia1formatado = moment
                    .utc(dia1semana)
                    .format("YYYY-MM-DD");
                  let mesmasemana = {
                    dia: dia1formatado,
      ...
      pontos_hora.map(p => {
        ...
        while (contador < 10) {
          ...
          for (let s = 0; s < p.npubs; s++) {
            if (p.pubs[s].userId == undefined) {
              ...
              for (let g = 0; g < this.usersesc.length; g++) {
                ...
                let idade = this.getAge(this.usersesc[g].datebirth);
                ...
                let dia1semanapre = moment.utc(this.dia).startOf("isoWeek");
                let dia1formatadopre = moment
                  .utc(dia1semanapre)
                  .format("YYYY-MM-DD");
                for (let m = 0; m < this.mesmasemana_agenda.length; m++) {
                  if (this.mesmasemana_agenda[m].id == this.usersesc[g].userId &&
                ...
                if (s > 0) {
                  ...
                  if (p.pubs[s - 1].conjuge && this.usersesc[g].conjuge) {
                    if (p.pubs[s - 1].userId == this.usersesc[g].conjuge &&
                      p.pubs[s - 1].conjuge == this.usersesc[g].userId)
                      casal = true;
                    else
                      continue;
                  }
                  /*      } else if ((p.pubs[s - 1].conjuge) && (p.pubs[s - 1].tipoesc != "A" && this.usersesc[g].conjuge == null) ) continue;
                                else if (p.pubs[s - 1].conjuge == null && this.usersesc[g].conjuge ) continue;
                   */
                  if (casal == false && p.pubs[s - 1].sex != this.usersesc[g].sex)
                    continue;
                }
                /*     if (s > 0) {
                                else if (p.pubs[s - 1].sex != this.usersesc[g].sex) continue;
                              } */
                ...
                for (let msem = 0; msem < this.mesmasemana.length; msem++) {
                  if (this.mesmasemana[msem].id == this.usersesc[g].userId &&
                ...
                for (let i = 0; i < config[dayoptionuser].length; i++) {
                  if (config[dayoptionuser][i].hora == h.code) {
                    ...
                    let dia1formatado = moment
                      .utc(dia1semana)
                      .format("YYYY-MM-DD");
                    let mesmasemana = {dia: dia1formatado, id: userh[0].userId};
                    this.mesmasemana.unshift(mesmasemana);
                    break;
                  }
                }
          ...
          if (p.pubs[0].userId != undefined && p.pubs[1].userId == undefined) {
            ...
            let year = this.getAge(p.pubs[0].datebirth);
            for (let g = 0; g < this.usersesc.length; g++) {
              if (p.pubs[0].userId == this.usersesc[g].userId) {
                let useralone = this.usersesc.splice(g, 1);
                this.usersesc.unshift(useralone[0]);
                let dia1semana = moment.utc(this.dia).startOf("isoWeek");
                let dia1formatado = moment.utc(dia1semana).format("YYYY-MM-DD");
                for (let m = 0; m < this.mesmasemana.length; m++) {
                  if (this.mesmasemana[m].id == p.pubs[0].userId &&
        ...
        /*     for (let s = 0; s < p.npubs; s++) {
                  if (p.pubs[s].userId != undefined) this.reportcount.push(p.pubs[s].userId);
                  if (s > 0) {
                    if ((p.pubs[0].userId == undefined) && (p.pubs[s].userId == undefined)) this.reportvoid++;
                  }
                } */
      });
      if (h.vagas > 0) {
      ...
      for (let u = 0; u < escala.pontos[p].length; u++) {
        for (let s = 0; s < escala.pontos[p][u].npubs; s++) {
          if (escala.pontos[p][u].pubs[s].userId == null ||
            ...
            /*   this.usersesc.map(j=> {
                        if(j.userId == escala.pontos[p][u].pubs[s].userId)j.lastday = new Date(Date.UTC(ano, mes, diadia, 3, 0, 0))
                      }); */
          }
        }
      }
  ...
  geraEscala() {
    ...
    this.users = usersort;
    this.usersesc = this.users;
    // antecipa quem esta agendado para nao ser escalado na semana
    this.mesmasemana_agenda = [];
    for (let i = 0; i < this.agendas.length; i++) {
      let dia1semana = moment.utc(this.agendas[i].data).startOf("isoWeek");
      let dia1formatado = moment.utc(dia1semana).format("YYYY-MM-DD");
      let mesmasemana_agenda = {
        dia: dia1formatado,
    ...
    while (this.diaatual < moment.utc(this.validity.end)) {
      ...
      // regra feriado fim
      this.pontos_dia = this.pontos.filter(a => a.config[this.dayoption].length > 0);
      this.carregaLayout();
    }
    for (let t = 0; t < this.users.length; t++) {
      let user = this.reportcount.filter(a => a == this.users[t].userId);
      let count = {
  ...
  salvaEscala() {
    let resposta = prompt("Confirma salvar a escala? Digite: salvar");
    if (resposta == "salvar") {
      if (this.escala.length > 0) {
        this.authService.saveEscala(this.escala).subscribe(data => {
  ...
  deletaEscala() {
    let resposta = prompt("Tem certeza que quer deletar a Escala? Digite: deletar");
    if (resposta == "deletar") {
      if (this.escala.length > 0) {
        this.authService.deleteEscala(this.datainicio).subscribe(data => {
  ...
  getEscala() {
    ...
    this.authService.getEscala(this.datainicio).subscribe(escala => {
      ...
      if (this.escala.length > 0) {
        ...
        this.authService.getAllLeds(this.datainicio).subscribe(leds => {
            ...
            for (let p = 0; p < this.escala[i].pontos.length; p++) {
              for (let u = 0; u < this.escala[i].pontos[p].length; u++) {
                for (let s = 0; s < this.escala[i].pontos[p][u].npubs; s++) {
                  if (this.escala[i].pontos[p][u].pubs[s] == null ||
                    this.escala[i].pontos[p][u].pubs[s] == undefined) {
                    this.escala[i].pontos[p][u].pubs[s] = new User();
                    if (s == 0)
                      this.reportvoid++;
                  } else {
                    this.reportcount.push(this.escala[i].pontos[p][u].pubs[s].userId);
                    let led = leds.find(a => {
  ...
  sendTelegram() {
    let resposta = prompt("Confirma envio de telegrams? Digite: telegram");
    if (resposta == "telegram") {
      if (this.escala.length > 0) {
        this.authService.sendTelegram(this.datainicio).subscribe(data => {
          console.log(data);
  ...
  sendEmail() {
    let resposta = prompt("Confirma envio de emails? Digite: email");
    if (resposta == "email") {
      if (this.escala.length > 0) {
        this.authService.sendEmails(this.datainicio).subscribe(data => {
          console.log(data);
            ...
            for (let p = 0; p < this.escala[i].pontos.length; p++) {
              ...
              for (let u = 0; u < this.escala[i].pontos[p].length; u++) {
                if (led)
                  break;
                for (let s = 0; s < this.escala[i].pontos[p][u].npubs; s++) {
                  if (this.escala[i].pontos[p][u].pubs[s].userId == data.iduser &&
                    ...
                    console.log(`led ${this.escala[i].pontos[p][u].pubs[s].firstName} modificado ${data.sim} ${data.nao}`);
                    led = true;
                    break;
                  }
                }
              ...
              for (let u = 0; u < this.escala[i].pontos[p].length; u++) {
                ...
                for (let s = 0; s < this.escala[i].pontos[p][u].npubs; s++) {
                  if (this.escala[i].pontos[p][u].pubs[s].userId == data.iduser &&
                    ...
                    console.log(`sub ${this.escala[i].pontos[p][u].pubs[s].firstName} modificado ${data.sim} ${data.nao}`);
                    led = true;
                    break;
                  }
                }
...
NewscheduleComponent.decorators = [
  {
    type: Component, args: [{
...
/** @nocollapse */
NewscheduleComponent.ctorParameters = () => [
  {type: AuthService,},
];

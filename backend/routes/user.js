...
if (req.body.responsable)responsable = mongoose.Types.ObjectId(req.body.responsable);
...
router.post('/signin', function(req, res, next) {
    ...
    User.findOne({email: req.body.email}, function(err, user) {
        ...
        if (user.role == 'super' ||
            user.role == 'pleno' ||
            user.role == 'gold' ||
            user.role == 'ctc')role = user.role;

        var token = jwt.sign({mykey: user._id, role: role }, 'secret', {expiresIn: 3600});
        return res.status(200).json({
...
router.put('/:id', function (req, res, next) {
    ...
    /*     if (req.params.id != decoded.mykey ) {
        ...
        //76
        let conjuge = null;
       if (req.body.conjuge == null || req.body.conjuge == undefined || req.body.conjuge == " "){
